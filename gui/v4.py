...
class TkMplSetup():
    ...
    def __init__(self, root):
        ...
        self.availableElecArrays= {'Medel - Sonnet 2', 'Cochlear - Nucleus 7'}
    ...
    def crearElementos1(self):
        ...
        dropdown = tk.OptionMenu(self.upperFrame, self.selectedElecArrayName, *self.availableElecArrays)
        ...
        #Play
        image = Image.open(r"C:\Users\Graciela\Desktop\Cochlear Implants\CodeGUI\playButton.png")
        image = image.resize((20,20))
        self.playImg = ImageTk.PhotoImage(image)

        #Stop
        image = Image.open(r"C:\Users\Graciela\Desktop\Cochlear Implants\CodeGUI\stopButton.png")
        image = image.resize((20,20))
        self.stopImg = ImageTk.PhotoImage(image)

        #Rec
        image = Image.open(r"C:\Users\Graciela\Desktop\Cochlear Implants\CodeGUI\recButton.png")
        image = image.resize((20,20))
    ...
    def crearElementos2(self):
        ...
        self.paramControls[DISPERTION_ID] = ttk.Scale(self.tab2, from_=0, to=2, orient= 'horizontal', command = self.slider_changed, variable = self.paramVals[DISPERTION_ID])
        self.paramControls[DISPERTION_ID].grid(column = 2, row = DISPERTION_ID+1, sticky = 'w')

        self.paramControls[MAXIMA_ID] = ttk.Scale(self.tab2, from_=6, to=12, orient= 'horizontal', command = self.slider_changed, variable = self.paramVals[MAXIMA_ID])
        self.paramControls[MAXIMA_ID].grid(column = 2, row = MAXIMA_ID+1, sticky = 'w')


        ##Displays de parámetros
        txt_Dispertion = ttk.Label(self.tab2, text = 'Dispertion: ')
        txt_Dispertion .grid(column = 0, row=DISPERTION_ID+1, sticky = 'w')
        self.paramTxts[DISPERTION_ID] = ttk.Label(self.tab2, text = '')
        self.paramTxts[DISPERTION_ID].grid(column = 1, row=DISPERTION_ID+1, sticky = 'w')


        txt_Maxima = ttk.Label(self.tab2, text = 'Maxima: ')
        txt_Maxima.grid(column = 0, row=MAXIMA_ID+1, sticky = 'w')
        self.paramTxts[MAXIMA_ID] = ttk.Label(self.tab2, text = 'Maxima: ')
        self.paramTxts[MAXIMA_ID].grid(column = 1, row=MAXIMA_ID+1, sticky = 'w')
    ...
    def openAudioEjemplo(self, temp):
        ...
        folder = r"C:\Users\Graciela\Desktop\Cochlear Implants\CodeGUI\demoSignals\\"
    ...
    def playStopSignal(self, s, sr):
        ...
        try:
            ...
            playing = audiostream.active
        except:
            pass #Aún no hay stream de audio
        ...
        if not playing:
            ...
            self.playOriginalButton.config(text='Stop', image = self.stopImg)

            #Seteo un timer para que cuando termine de reproducirse cambie el texto
            self.dur = len(s)/sr #duracion en segundos
            t = threading.Timer(self.dur, stopSignal)   # task will trigger after 60 seconds
            t.start()
    ...
    def start_visualisation(self):
        ...
        self.start_time = time.time()
        self.pause_time = 0
        #self.time_progress['maximum'] = Lo que quieras
    ...
    def update_frame(self, frame):
        ...
        #Todo: Chequear si está reproduciendo audio. Si no, return
